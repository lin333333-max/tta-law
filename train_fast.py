...
from fast_data_loader import get_fast_dataloader  # 使用快速加载器
from data_loader import get_dataloader, MappingDictionary  # 保留原始加载器作为备用
...
def main():
    ...
    if config.use_cached_data and os.path.exists(config.cache_dir):
        ...
        # 使用原始数据加载（慢速模式）
        print("⚠ Using original data loading (Slow mode)")
        print("  Run 'python preprocess_data.py' to generate cache for faster training")
